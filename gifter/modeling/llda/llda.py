...
class LLDA:
    ...
    def inference(self):
        V = len(self.vocas)
        for m, doc, label in zip(range(len(self.docs)),
            ...
            for n in range(len(doc)):
                ...
                p_z = label * \
                    (self.n_z_t[:, t] + self.beta) / denom_b * \
                    (self.n_m_z[m] + self.alpha) / denom_a
    ...
    def phi(self):
        V = len(self.vocas)
        return (self.n_z_t + self.beta) / (self.n_z[:, numpy.newaxis] + V *
                                           self.beta)
...
def main():
    ...
    parser.add_option(
        ...
        default=0.001)
    parser.add_option(
        "-k", dest="K", type="int", help="number of topics", default=20)
    parser.add_option(
        "-i", dest="iteration", type="int", help="iteration count",
        default=100)
